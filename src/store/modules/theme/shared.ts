...
export function initThemeSettings() {
  ...
  if (!isProd) {
    // in dev mode, restore themeScheme from localStorage so it persists across refresh
    const cachedScheme = localStg.get('themeScheme');
    if (cachedScheme) {
      return { ...themeSettings, themeScheme: cachedScheme };
    }
