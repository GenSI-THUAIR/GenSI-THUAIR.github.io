...
export function getAllTabs(tabs: App.Global.Tab[], homeTab?: App.Global.Tab) {
  ...
  // 检查是否已经存在相同路由的标签页，如果存在则不重复显示homeTab
  const hasMatchingTab = tabs.some(tab => tab.routeKey === homeTab.routeKey);
  
  const allTabs = hasMatchingTab ? [...fixedTabs, ...remainTabs] : [homeTab, ...fixedTabs, ...remainTabs];
...
export function getDefaultHomeTab(router: Router, homeRouteName: LastLevelRouteKey) {
  ...
  if (homeRoute) {
    ...
    // 强制移除图标，即使从路由中获取了图标
    homeTab.icon = undefined;
    homeTab.localIcon = undefined;
  }
