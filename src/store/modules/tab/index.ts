...
export const useTabStore = defineStore(SetupStoreId.Tab, () => {
  ...
  function initHomeTab() {
    // 如果 routeHome 是 portal_home（门户首页），则不在后台创建 home 标签
    // 因为后台应该直接进入 news-manage
    if (routeStore.routeHome === 'portal_home') {
  ...
  function isTabRetain(tabId: string) {
    // 如果没有 homeTab（即 routeHome 是 news-manage），则只检查固定标签
    if (homeTab.value && tabId === homeTab.value.id) return true;
